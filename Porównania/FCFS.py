...
poczatkowa_liczba_procesow = 40  # Poczatkowa liczba procesow do wykonania
...
with open('FCFS_wyniki.txt', 'w') as plik:
    ...
    while len(wygenerowane_procesy) > 0:
        ...
        if wygenerowane_czasy_procesora[0] > 1:  # Wykonywanie jednej jednostki czasu dla wykonywanego procesu
            wygenerowane_czasy_procesora[0] -= 1
            ...
            if len(wygenerowane_procesy) > 1:  # Doliczenie czasu procesora z powodu zmiany procesu
                czas_zmiany_procesu = abs(wygenerowane_procesy[0] - wygenerowane_procesy[1])
            czas_pracy_procesora += 0.1 * czas_zmiany_procesu
            wygenerowane_procesy.pop(0)
            wygenerowane_czasy_procesora.pop(0)
            średni_czas_oczekiwania_na_wykonanie += tablica_oczekiwania_pojedynczego_procesu[0]
            tablica_oczekiwania_pojedynczego_procesu.pop(0)
            czas_pojedynczego_procesu = 0
            pomocnicza -= 1
    ...
    plik.write("\nSredni czas oczekiwania zadania: " + str(średni_czas_oczekiwania_na_wykonanie) + "[s]")
